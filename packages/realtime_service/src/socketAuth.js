...
export function createSocketAuthMiddleware(config, logger,prisma) {
  ...
  return async (socket, next) => {
    try {
      ...
      if (!userRecord || userRecord.deletedAt) {
        return next(new Error("Account is scheduled for deletion"));
      }
...
export function createVerificationSocketMiddleware(prisma, logger) {
  return async (socket, next) => {
    try {
      ...
      logger.error(
        { err: err.message, userId: socket.userId },
