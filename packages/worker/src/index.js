...
import {
  startFeedSchedulerWorker,
  scheduleFeedSchedulerQueueJobRepeatable,
} from "./workers/feedScheduleWorker.js";
...
async function main() {
  ...
  const {
    ...
    breakerRedisConnection: verificationBreakerRedis,
  } = startVerificationWorker(logger);
  const {
    worker: matchNotificationWorker,
    connection: matchNotificationConnection,
    breakerRedisConnection: matchNotificationBreakerRedis,
  } = startMatchNotificationWorker(logger);
  const {
    worker: feedRefillWorker,
    connection: feedRefillConnection,
    listRedis: feedListRedis,
  } = await startFeedRefilWorker(logger);
  const {
    worker: feedSchedulerWorker,
    connection: feedSchedulerConnection,
    enqueueConnection,
  } = await startFeedSchedulerWorker(logger);
  const { worker: iceBreakerWorker, connection: iceBreakConnection } =
    await startIceBreakerFunction(logger);
  const {
    worker: locationShareCleanupWorker,
    connection: locationShareCleanupConnection,
  } = startLocationCleanUpWorker(logger);
  const {
    worker: embeddingWorker,
    connection: embeddingConnection,
    breakerRedisConnection: embeddingBreakerRedis,
  } = startEmbeddingWorkerForProfile(logger);
  const {
    worker: scamDetectionWorker,
  ...
  logger.info(
    "Worker process started, listening for jobs on: health-check, verification-check",
  );
